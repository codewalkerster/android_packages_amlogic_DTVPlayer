...
public class DvbtScanResult extends DTVActivity{

	private final String TAG = "DvbtScanResult";
	...
	private int service_get_dbId(String name, int serviceid, int serviceType){
		...
		TVProgram mTVProgram = TVProgram.selectByNameAndServiceId(DvbtScanResult.this,name,serviceid,serviceType);
	...
	public void onConnected(){
		...
		if(bundle!=null){
    		if(bundle.getString("scan-mode").equals("dvbt-auto-scan")){
	...
	private void DTVScanDVBC_StartManuScan(int fre,int modulation,int symbole){	
		TVScanParams sp=TVScanParams.dtvManualScanParams(0, TVChannelParams.dvbcParams(fre*1000, modulation, symbole));
		startScan(sp);
	}

	private void DTVScanDVBT_StartManuScan(int band_width,int fre)
	{
		Log.d(TAG, "DTVScanDVBT_StartManuScan");
	...
	private void process_event_scan_service(TVMessage msg){
		...
		if(name!=null){
			...
			if(service_type==1){			
				...
				mTvListAdapter = new ScanResultAdapter(DvbtScanResult.this,tv_list);
				tvlistview.setAdapter(mTvListAdapter);
