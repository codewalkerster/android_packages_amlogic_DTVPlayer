...
public class DvbsScanResult extends DvbActivity{

	private final String TAG = "DvbsScanResult";
	private static final int DVB_SEARCH_MODULES= 18;
	private static final String DVB_SERVICE_INTERFACE = "com.amlogic.dvb.DVB_SERVICE";
	...
	@Override
  	public void onCreate(Bundle savedInstanceState) {
			...
			try
			{
				...
				"com.amlogic.DVBPlayer", Context.MODE_WORLD_WRITEABLE|Context.MODE_WORLD_READABLE);
			}
	...
	private void playProgram(int type,int pos){
		...
		ComponentName comp = new ComponentName("com.amlogic.DVBPlayer","com.amlogic.DVBPlayer.DVBPlayer");
	...
	private DVBClient mScanDvb = null;
	protected void onStart()
	{
		super.onStart();
		sync_flag=false;
		mScanDvb = new DVBClient(this, DvbsScanResult.DVB_SERVICE_INTERFACE);
		...
		if(ret<0)
		  ...
		  mScanDvb.setEventListener(new OnEventListener() {
			public void onEvent(DVBClient c, DVBEvent e) {
				Log.d( TAG, " onEvt: " + e.type + " " + e.msg);
				Handler mHandler = new Handler(Looper.getMainLooper()) {
					public void handleMessage(Message msg) {
						if(msg.what == DVB_SEARCH_MODULES)
							ResultOnEvent((DVBEvent)msg.obj);
					}
				};
				Message m = mHandler.obtainMessage(DVB_SEARCH_MODULES, e);
				mHandler.sendMessage(m);
			}
			public void onConnected(DVBClient c) {
				Log.d(TAG, "onConnected");

				if(ScanConfigActivity.getDefaultList()!=null){
					Log.d(TAG,"####"+ScanConfigActivity.getDefaultList().size());	
					for(int i=0;i<ScanConfigActivity.getDefaultList().size();i++){
						DVBSatelliteParameter sat_para = ScanConfigActivity.getDefaultList().get(i).sat;
						Log.d(TAG,"sat name:"+sat_para.name);

						ArrayList<DVBFrontEndPara> ts_list=ScanConfigActivity.getDefaultList().get(i).ts_list;	

						if(ts_list!=null){
							Log.d(TAG,"ts number:"+ts_list.size());
							for(int j=0;j<ts_list.size();j++){
								DVBFrontEndPara ts_node = ts_list.get(j);
				...
				if(mLast.getString("scan_mode","default").equals("default")){
					...
					if(mSatScanCount<ScanConfigActivity.getDefaultList().size()){
						...
						if(mSatScanCount<ScanConfigActivity.getDefaultList().size()){	
							...
							ScanConfigActivity.getDefaultList().get(mSatScanCount).ts_list,
							 false,ScanConfigActivity.getDefaultList().get(mSatScanCount).sat);
	...
	private void process_event_scan_service(DVBEvent evt)
	{
		...
		Log.d(TAG,"sat name"+ScanConfigActivity.getDefaultList().get(mSatScanCount).sat.name);
		...
		//serviceinfo.setSatName(ScanConfigActivity.getDefaultList().get(mSatScanCount).sat.name);
	...
	private void process_event_scan_signal_info(DVBEvent evt){
	...
	private void returnSettings(){
		...
		intent.setClass(DvbsScanResult.this, ScanConfigActivity.class);
